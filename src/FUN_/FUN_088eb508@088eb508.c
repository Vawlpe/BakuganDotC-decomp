#include "ULUS10536_MYTHREAD-MAIN.BIN.h"

int FUN_088eb508(int param_1)

{
  FUN_089bf238(param_1,0);
  *(undefined **)(param_1 + 0xc) = &DAT_08af41ec;
  *(undefined4 *)(param_1 + 0x20) = 0;
  *(undefined4 *)(param_1 + 0x24) = 0;
  *(undefined4 *)(param_1 + 0x28) = 0;
  *(undefined4 *)(param_1 + 0x2c) = 0;
  *(undefined4 *)(param_1 + 0x30) = 0;
  *(undefined4 *)(param_1 + 0x34) = 0;
  *(undefined4 *)(param_1 + 0x38) = 0;
  *(undefined4 *)(param_1 + 0x3c) = 0;
  *(undefined4 *)(param_1 + 0x40) = 0;
  *(undefined4 *)(param_1 + 0x44) = 0;
  *(undefined4 *)(param_1 + 0x48) = 0;
  *(undefined4 *)(param_1 + 0x4c) = 0;
  *(undefined2 *)(param_1 + 0x250) = 0;
  *(undefined2 *)(param_1 + 0x252) = 0;
  *(undefined2 *)(param_1 + 0x254) = 0;
  *(undefined2 *)(param_1 + 0x256) = 0;
  *(undefined2 *)(param_1 + 600) = 0;
  *(undefined2 *)(param_1 + 0x25a) = 0;
  *(undefined2 *)(param_1 + 0x25c) = 0;
  *(undefined2 *)(param_1 + 0x25e) = 0;
  *(undefined2 *)(param_1 + 0x260) = 0;
  *(undefined2 *)(param_1 + 0x262) = 0;
  *(undefined *)(param_1 + 0x264) = 0;
  *(undefined *)(param_1 + 0x265) = 0;
  *(undefined *)(param_1 + 0x267) = 0;
  *(byte *)(param_1 + 0x273) = *(byte *)(param_1 + 0x273) & 0xfe;
  *(undefined *)(param_1 + 0x268) = 2;
  *(undefined *)(param_1 + 0x269) = 0;
  *(undefined *)(param_1 + 0x26a) = 0;
  *(undefined *)(param_1 + 0x26b) = 0xf0;
  *(undefined *)(param_1 + 0x26c) = 0xf0;
  *(undefined *)(param_1 + 0x26d) = 0;
  *(undefined *)(param_1 + 0x26e) = 0;
  *(undefined *)(param_1 + 0x26f) = 0;
  *(byte *)(param_1 + 0x273) = *(byte *)(param_1 + 0x273) & 0xfd;
  *(undefined *)(param_1 + 0x270) = 0;
  *(undefined *)(param_1 + 0x271) = 0xff;
  *(undefined *)(param_1 + 0x272) = 0;
  *(byte *)(param_1 + 0x273) = *(byte *)(param_1 + 0x273) | 4;
  *(byte *)(param_1 + 0x273) = *(byte *)(param_1 + 0x273) & 0xf7;
  memset_jak(param_1 + 0x10,0,0x10);
  memset_jak(param_1 + 0x50,0,0x100);
  memset_jak(param_1 + 0x150,0,0x100);
  return param_1;
}

